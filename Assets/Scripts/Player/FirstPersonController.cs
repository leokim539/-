...
public class FirstPersonController : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    void MovePlayer()
    {
        ...
        bool isMoving = Mathf.Abs(x) > 0 || Mathf.Abs(z) > 0; // 방향키 입력 여부 확인
        bool isSprinting = isMoving && Input.GetKey(runningKey) && stamina > 0; // 스프린트 여부 체크

        float targetMovingSpeed = isSprinting ? runSpeed : moveSpeed;

        if (isSprinting) // 스프린트 시 스태미나 소모
        {
            stamina -= staminaDrainRate * Time.deltaTime; // 초당 1씩 감소
